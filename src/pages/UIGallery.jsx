...
import { uiGallery } from "../constants";
  ...
  return (
    <Container className="font-Poppins">
      ...
      <div className="px-4 sm:px-0 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6  my-16">
        {uiGallery.map((font, index) => (
          <figure key={font.name}>
            {/* Modal Full gambar */}
            {showModal && (
              <div className="z-50 fixed inset-0 bg-Black/70 flex justify-center items-center">
                <Carousel className="h-2/6 sm:h-3/6 lg:h-4/6 w-5/6 sm:w-4/6 lg:w-3/6 bg-black relative">
                  ...
                  <CarouselContent>
                    <CarouselItem>
                      {" "}
                      <img
                        src={uiGallery[indexModalActive].image}
                        alt={uiGallery[indexModalActive].name}
