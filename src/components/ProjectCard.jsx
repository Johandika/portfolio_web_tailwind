  ...
  return (
    <div className={`max-w-5xl min-h-[400px] sm:h-fit w-full ring-white ring-1 flex flex-col sm:flex-row items-center rounded-lg  hover:shadow-2xl ${shadowBlue? 'hover:shadow-blue-500':shadowGreen? 'hover:shadow-green-500': 'hover:shadow-RedDarkest'} hover:shadow-RedDarkest transition overflow-hidden cursor-pointer group`}>
        ...
        <div className="w-full order-1 sm:order-2 sm:w-1/2 relative">
          <div className={`flex justify-center items-center rounded-lg rotate-12 relative ring-1 ring-gray-200 group-hover:shadow-2xl

          ${shadowBlue? 'group-hover:shadow-blue-500'
            :shadowGreen? 'group-hover:shadow-green-500'
            : 'group-hover:shadow-RedDarkest'} 

            z-0 -right-8 -bottom-10 overflow-hidden scale-100 hover:scale-105 transition`}>
              
            <div className="w-full h-full sm:bg-Black sm:group-hover:bg-Black/0 transition z-10 absolute">
            </div>
            <img src={image} alt={client} className="w-full object-cover" />
          </div>
        </div>
      </div>
  )
